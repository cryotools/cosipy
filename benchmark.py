...
def main():
    ...
    if Config.slurm_use:
        SlurmConfig()
        with SLURMCluster(job_name=SlurmConfig.name, cores=SlurmConfig.cores, processes=SlurmConfig.cores,
                memory=SlurmConfig.memory, account=SlurmConfig.account,
                job_extra_directives=SlurmConfig.slurm_parameters,
                local_directory=SlurmConfig.local_directory, ) as cluster:
        ...
        run_cosipy(None, IO, DATA, RESULT, RESTART, futures)
...
def run_cosipy(cluster, IO, DATA, RESULT, RESTART, futures):
        ...
        if Config.WRF:
            ...
            if (mask == 1) & (not Config.restart):
                if np.isnan(DATA.sel(south_north=y, west_east=x).to_array()).any():
                    ...
                    sys.exit()
                futures.append(cosipy_core(DATA.sel(south_north=y, west_east=x), y, x, stake_names=stake_names,
                                           stake_data=df_stakes_data))
            elif (mask == 1) & (Config.restart):
                if np.isnan(DATA.sel(south_north=y, west_east=x).to_array()).any():
                    print('ERROR!!!!!!!!!!! There are NaNs in the dataset')
                    sys.exit()
                futures.append(cosipy_core(DATA.sel(south_north=y, west_east=x), y, x,
            ...
            if (mask == 1) & (not Config.restart):
                if np.isnan(DATA.isel(lat=y, lon=x).to_array()).any():
                    ...
                    sys.exit()
                futures.append(
                    cosipy_core(DATA.isel(lat=y, lon=x), y, x, stake_names=stake_names, stake_data=df_stakes_data))
            elif (mask == 1) & (Config.restart):
                if np.isnan(DATA.isel(lat=y, lon=x).to_array()).any():
                    print('ERROR!!!!!!!!!!! There are NaNs in the dataset')
                    sys.exit()
                futures.append(
                    cosipy_core(DATA.isel(lat=y, lon=x), y, x, GRID_RESTART=IO.create_grid_restart().isel(lat=y, lon=x),
                                stake_names=stake_names, stake_data=df_stakes_data))
    ...
    for future in futures:
        ...
        indY, indX, local_restart, RAIN, SNOWFALL, LWin, LWout, H, LE, B, QRR, MB, surfMB, Q, SNOWHEIGHT, TOTALHEIGHT, TS, ALBEDO, NLAYERS, ME, intMB, EVAPORATION, SUBLIMATION, CONDENSATION, DEPOSITION, REFREEZE, subM, Z0, surfM, MOL, LAYER_HEIGHT, LAYER_RHO, LAYER_T, LAYER_LWC, LAYER_CC, LAYER_POROSITY, LAYER_ICE_FRACTION, LAYER_IRREDUCIBLE_WATER, LAYER_REFREEZE, stake_names, stat, df_eval = future

        IO.copy_local_to_global(indY, indX, RAIN, SNOWFALL, LWin, LWout, H, LE, B, QRR, MB, surfMB, Q, SNOWHEIGHT,
                                TOTALHEIGHT, TS, ALBEDO, NLAYERS, ME, intMB, EVAPORATION, SUBLIMATION, CONDENSATION,
                                DEPOSITION, REFREEZE, subM, Z0, surfM, MOL, LAYER_HEIGHT, LAYER_RHO, LAYER_T, LAYER_LWC,
...
if __name__ == "__main__":
    ...
    main()
    print(f"_____ Peak RAM: {peak_ram} MB")
