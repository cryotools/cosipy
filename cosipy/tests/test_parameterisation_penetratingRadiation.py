...
class TestParamRadiation:
    ...
    def test_penetrating_radiation_error(
        ...
        conftest_boilerplate.patch_variable(
            monkeypatch, pRad.cc, {"penetrating_method": "WrongMethod"}
        )
        assert pRad.cc.penetrating_method == "WrongMethod"
        error_msg = (
            f'Penetrating method = "{pRad.cc.penetrating_method}" ',
    ...
    @pytest.mark.parametrize(
        "arg_density", [250.0, cc.snow_ice_threshold + 1]
    )
    def test_method_Bintanja(
        ...
        conftest_boilerplate.patch_variable(
            monkeypatch, pRad.cc, {"penetrating_method": "Bintanja95"}
        )
        assert pRad.cc.penetrating_method == "Bintanja95"
        test_grid = conftest_mock_grid
        test_grid.add_fresh_snow(0.1, arg_density, 270.15, 0.0, 0.0)
        test_swnet = 800.0
        if arg_density <= cc.snow_ice_threshold:
            test_si = test_swnet * 0.1
        else:
            test_si = test_swnet * 0.2

        melt_si = pRad.method_Bintanja(
            GRID=test_grid, SWnet=test_swnet, dt=cc.dt
        )
