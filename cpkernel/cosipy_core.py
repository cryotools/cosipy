...
def init_nan_array(nt: int,ct) -> np.ndarray:
    ...
    if not Config.WRF_X_CSPY:
        y = np.full(nt, np.nan)
        x = xr.DataArray(y, coords = [('time',ct)])
    else:
        x = None
...
def cosipy_core(DATA, indY, indX, lat_val, lon_val, zarr_path, GRID_RESTART=None, stake_names=None, stake_data=None):
    ...
    """
    if zarr_path == None: 
        return cosipy_core_old(DATA, indY, indX, GRID_RESTART=None, stake_names=None, stake_data=None)
    ...
    """
    if not WRF_X_CSPY:
        _RRR = init_nan_array(nt,ct)
        _RAIN = init_nan_array(nt,ct)
        _SNOWFALL = init_nan_array(nt,ct)
        _LWin = init_nan_array(nt,ct)
        _LWout = init_nan_array(nt,ct)
        _H = init_nan_array(nt,ct)
        _LE = init_nan_array(nt,ct)
        _B = init_nan_array(nt,ct)
        _QRR = init_nan_array(nt,ct)
        _MB = init_nan_array(nt,ct)
        _surfMB = init_nan_array(nt,ct)
        _MB = init_nan_array(nt,ct)
        _Q = init_nan_array(nt,ct)
        _SNOWHEIGHT = init_nan_array(nt,ct)
        _TOTALHEIGHT = init_nan_array(nt,ct)
        _TS = init_nan_array(nt,ct)
        _ALBEDO = init_nan_array(nt,ct)
        _ME = init_nan_array(nt,ct)
        _intMB = init_nan_array(nt,ct)
        _EVAPORATION = init_nan_array(nt,ct)
        _SUBLIMATION = init_nan_array(nt,ct)
        _CONDENSATION = init_nan_array(nt,ct)
        _DEPOSITION = init_nan_array(nt,ct)
        _REFREEZE = init_nan_array(nt,ct)
        _NLAYERS = init_nan_array(nt,ct)
        _subM = init_nan_array(nt,ct)
        _Z0 = init_nan_array(nt,ct)
        _surfM = init_nan_array(nt,ct)
        _MOL = init_nan_array(nt,ct)
        _new_snow_height = init_nan_array(nt,ct)
        _new_snow_timestamp = init_nan_array(nt,ct)
        _old_snow_timestamp = init_nan_array(nt,ct)
    ...
    LWin_v = np.array(nt * [None])
    N_v = np.array(nt * [None])
    if ('LWin' in DATA) and ('N' in DATA):
        LWin_v = DATA.LWin.values
        N_v = DATA.N.values
    elif 'LWin' in DATA:
        LWin_v = DATA.LWin.values
    else:
    ...
    for t in np.arange(nt):
        ...
        if (LWin_v is not None) and (N_v is not None):
            ...
            fun, surface_temperature, lw_radiation_in, lw_radiation_out, sensible_heat_flux, latent_heat_flux, \
                 ...
                 U2[t], RAIN, SLOPE, LWin=LWin_v[t], N=N_v[t])
        elif LWin_v is not None:
            fun, surface_temperature, lw_radiation_in, lw_radiation_out, sensible_heat_flux, latent_heat_flux, \
                ground_heat_flux, rain_heat_flux, rho, Lv, MOL, Cs_t, Cs_q, q0, q2 \
                = update_surface_temperature(GRID, dt, z, z0, T2[t], RH2[t], PRES[t], sw_radiation_net, \
                                             U2[t], RAIN, SLOPE, LWin=LWin_v[t])
        else:
            # Find new surface temperature (LW is parametrized using cloud fraction)
            fun, surface_temperature, lw_radiation_in, lw_radiation_out, sensible_heat_flux, latent_heat_flux, \
                ground_heat_flux, rain_heat_flux, rho, Lv, MOL, Cs_t, Cs_q, q0, q2 \
                = update_surface_temperature(GRID, dt, z, z0, T2[t], RH2[t], PRES[t], sw_radiation_net, \
                                             U2[t], RAIN, SLOPE, N=N_v[t])
        ...
        if not WRF_X_CSPY:
            if LWin_v is not None:
                LWin_v[t] = lw_radiation_in
            ...
            _TS[t] = surface_temperature.item()
            _ALBEDO[t] = alpha
    ...
    if not WRF_X_CSPY:
        ...
        region_to_write = {'lat': slice(indY, indY + 1),'lon': slice(indX, indX + 1), 'time': slice(0,nt)}#Distributed writing to a single zarr file
        RESULT['RAIN'] = _RAIN
        ...
        RESULT['QRR'] = _QRR
        RESULT['MB'] = _MB
        RESULT['surfMB'] = _surfMB
        RESULT['Q'] = _Q
        RESULT['SNOWHEIGHT'] = _SNOWHEIGHT
        RESULT['TOTALHEIGHT'] = _TOTALHEIGHT
        RESULT['TS'] = _TS
        RESULT['ALBEDO'] = _ALBEDO
        RESULT['NLAYERS'] = _NLAYERS
        RESULT['ME'] = _ME
        RESULT['intMB'] = _intMB
        RESULT['EVAPORATION'] = _EVAPORATION
        RESULT['SUBLIMATION'] = _SUBLIMATION
        RESULT['CONDENSATION'] = _CONDENSATION
        RESULT['DEPOSITION'] = _DEPOSITION
        RESULT['REFREEZE'] = _REFREEZE
        RESULT['subM'] = _subM
        RESULT['Z0'] = _Z0
        RESULT['surfM'] = _surfM
        RESULT['NEWSNOWHEIGHT'] = _new_snow_height
        RESULT['NEWSNOWTIMESTAMP'] = _new_snow_timestamp
        RESULT['OLDSNOWTIMESTAMP'] = _old_snow_timestamp
        RESULT = RESULT.expand_dims({"lat": [lat_val], "lon": [lon_val]})

        RESULT.to_zarr(zarr_path,mode ='r+',region = region_to_write, safe_chunks = True,write_empty_chunks=True)
...
def cosipy_core_old(DATA, indY, indX, GRID_RESTART=None, stake_names=None, stake_data=None):
    """Cosipy core function.
