...
def init_result_dataset(DATA):
    ...
    add_variable_2D(RESULT, np.full_like(DATA.T2, np.nan, dtype=np.double), 'LWnet', 'W m\u207B\xB2', 'net longwave radiation')
...
def init_result_dataset_1D(DATA):
    ...
    add_variable_1D(RESULT, np.full_like(DATA.T2, np.nan, dtype=np.double), 'LWnet', 'W m\u207B\xB2', 'net longwave radiation')
